#!/usr/bin/env Rscript
# This script pulls out all samples where our ASV of interest is present and then calculates the Spearman correlation
  ...
  for (i in 1:nrow(positive_results)) {
    ...
    if (asv_name %in% colnames(ASV_union_subset) && ASV_of_interest %in% colnames(ASV_union_subset)) {
      ...
      p <- ggplot(plot_data, aes(x = asvi, y = Correlated_ASV)) +
        ...
        labs(
          title = paste("Correlation between", ASV_of_interest, "and", asv_name),
          x = ASV_of_interest,
          y = asv_name
        ) +
        annotate("text",
          x = max(plot_data$asvi) * 0.8, y = max(plot_data$Correlated_ASV) * 0.9,
          label = paste("Spearman's rho = ", round(spearman_corr, 2)), color = "red", size = 5
